...
void copia_file(char* path1,char* path2){
	...
	fdW = open (path2, O_WRONLY | O_CREAT , S_IRUSR | S_IWUSR);		//SOVRASCRIVI
	if (fdR==(-1) || fdW==(-1)) {
		...
		if(strcmp(dir1P->d_name,".")!=0&&strcmp(dir1P->d_name,"..")!=0){
			...
			if(dir1P->d_type==DT_DIR){
				...
				while(dir2P=readdir(dir2)!=NULL){
					if(strcmp(dir2P->d_name,".")!=0&&strcmp(dir1P->d_name,"..")!=0){
						if(strcmp(dir1P->d_name,dir2P->d_name)==0){
							printf("E' presente un duplicato: preferisci tenere il file %s (premi 0) o il file %s (premi 1)?",next_path1,next_path2);
							do{
								scanf("%d",&choice);
								if(choice==0)
									copia_file(next_path1,next_path2);	//SOVRASCRIVE
								else if(choice==1){
									break;
								}
							}
